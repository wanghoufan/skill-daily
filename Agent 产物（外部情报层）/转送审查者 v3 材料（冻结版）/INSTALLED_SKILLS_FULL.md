...
> 更新时间：2026-09-22 15:12 (GMT+8，全量重扫)
> 数据基准：2026-09-22 15:12 全量重扫（扫描两遍目录状态一致；替代 13:55 旧基准，13:55 后外部工具造成的 3 个 Orca 主副本移除与 7 个『(2)』重复目录已纳入本报告）
...
- `~/.skills-manager/skills/` — 中央仓库（用户级，46 个含 SKILL.md 的目录，各 agent 的规范加载源；含 7 个『(2)』重复目录，见下）
...
第四轮重扫记录（冻结前）：① 13:55 后外部工具移除中央 orca-cli/orchestration/computer-use 主副本 → 6 条链接悬空，三项移出安装集（来源档案保留）；② 新增 7 个『(2)』重复目录（内容与原目录逐字节一致，来源继承）；③ 精确化版本关系表述（VERSION_STATUS）：来源确认不要求所有 Skill 与当前上游逐字节一致——Orca 系=与官方历史版本一致（historical_official_version），browseros-neo/aihot=落后当前上游（outdated_version），hv-analysis/khazix-writer=与 khazix-skills 当前一致（upstream_in_sync）；版本旧不触发 LOCAL_MODIFIED。
第一/二轮修订记录：第一版曾因『面向 Orca/BrowserOS』判 vendor、『调用 aihot API』判 community，第二证据不足降回 unknown；第三轮以联网实证重新升级（见上）。

- 已确认来源（作者归属有证据）：**12**（official 5 ｜ vendor 1 ｜ community 2 ｜ local 4 ｜ fork 0 ｜ modified 0）
- 来源未知：**37**（orca-codex-commands 已排除官方发布；leader/neat-freak 有逐字节一致证据但待用户确认，见 pending_upgrade）
...
- **browseros-neo** ← github.com/browseros-ai/BrowserOS ｜ confidence=medium ｜ 证据：安装目录含 BrowserOS 官方管理清单 .browserclaw-managed.json（官方安装器管理标记）；内容与官方仓库 skill 高度同源，但与所查 3 个官方 commit 均非逐字节一致（官方持续演进）→ 判 official，confidence=medium。
  - 版本关系：outdated_version — 与所查 3 个官方 commit 均非逐字节一致：官方持续演进、本地版本未跟进（由官方安装器管理）。与当前上游不一致 ≠ 本地修改。
  - 活跃度：active（核验日 2026-09-22；GitHub API 实测 browseros-ai/BrowserOS 最近 push 2026-09-22）
- **computer-use**　⚠️ 当前未安装（2026-09-22 被外部工具移除，档案保留） ← github.com/stablyai/orca ｜ confidence=high ｜ 证据：内容与官方仓库逐字节一致：本地 SKILL.md（c79dedb3c7df）== stablyai/orca commit 1a9e819c（2026-07-22）中 skills/computer-use/SKILL.md → 由 Orca 官方仓库发布。
  - 版本关系：historical_official_version — 与官方历史 commit 1a9e819c 逐字节一致；官方仓库此后有更新。版本旧 ≠ 本地修改。
...
- **computer-use-2** ← github.com/stablyai/orca ｜ confidence=high ｜ 证据：本地目录名为改名副本，但内容未改动：SKILL.md（991f5dcdb2e5）与 stablyai/orca commit b44ef1e5（2026-08-31）中 skills/computer-use/SKILL.md 逐字节一致 → 内容源自官方仓库（非本地修改；目录改名不改变来源归属）。
  - 版本关系：historical_official_version — 与官方 skills/computer-use@b44ef1e5（2026-08-31）逐字节一致，是所查官方版本中最新的一个；本地仅目录改名。
  - 活跃度：active（核验日 2026-09-22；GitHub API 实测 stablyai/orca 最近 push 2026-09-22）
...
- **orca-cli**　⚠️ 当前未安装（2026-09-22 被外部工具移除，档案保留） ← github.com/stablyai/orca ｜ confidence=high ｜ 证据：内容与官方仓库逐字节一致：本地 SKILL.md（sha1 前12位 cddfd0a8c44e）== stablyai/orca commit f8b430f7（2026-07-20）中 skills/orca-cli/SKILL.md，同算法实测 → 由 Orca 官方仓库发布（第二轮『仅面向产品』的保守判定据此撤销）。
  - 版本关系：historical_official_version — 与官方历史 commit f8b430f7 逐字节一致；官方仓库此后有更新（2026-08-31 后的 computer-use 基线），本地停留在历史版本。版本旧 ≠ 本地修改。
  - 活跃度：active（核验日 2026-09-22；GitHub API 实测 stablyai/orca 最近 push 2026-09-22）
- **orchestration**　⚠️ 当前未安装（2026-09-22 被外部工具移除，档案保留） ← github.com/stablyai/orca ｜ confidence=high ｜ 证据：内容与官方仓库逐字节一致：本地 SKILL.md（8dd31dae1235）== stablyai/orca commit 1a9e819c（2026-07-22）中 skills/orchestration/SKILL.md → 由 Orca 官方仓库发布。
  - 版本关系：historical_official_version — 与官方历史 commit 1a9e819c 逐字节一致；官方仓库此后有更新。版本旧 ≠ 本地修改。
  - 活跃度：active（核验日 2026-09-22；GitHub API 实测 stablyai/orca 最近 push 2026-09-22）
...
- **aihot** ← Virxact / AI HOT（aihot.virxact.com） ｜ confidence=high ｜ 证据：SKILL.md frontmatter 直接署名：metadata.author=Virxact、version=1.2.0（文件内作者证据，非『调用其 API』推断）；官方渠道当前版本 v1.7.1 同署名，版本谱系连续 → 由 AI HOT 服务方 Virxact 发布。
  - 版本关系：outdated_version — 本地 v1.2.0 落后官方渠道 v1.7.1（版本谱系连续、署名一致）。版本旧 ≠ 本地修改。
...
- **hv-analysis** ← 数字生命卡兹克 (Khazix) — github.com/KKKKhazix/khazix-skills ｜ confidence=high ｜ 证据：SKILL.md 明确标注方法由『数字生命卡兹克』提出；且内容与 github.com/KKKKhazix/khazix-skills 仓库中同名 skill 逐字节一致（本地 cb6227bf8ee4 == 上游实测）→ 作者归属与上游均确认，confidence 升为 high。
  - 版本关系：upstream_in_sync — 与 KKKKhazix/khazix-skills 当前同名 skill 逐字节一致。
...
- **skill-manager-governance (2)** ← 本地 Skill Manager / 60 Skill 仓库 ｜ confidence=high ｜ 证据：『(2)』重复目录（2026-09-22 15:12 重扫发现，外部工具产生）：SKILL.md 内容与『skill-manager-governance』原目录逐字节一致（sha1 前12位同值），来源继承原目录判定；目录名含空格括号，目录扫描型加载器会将其识别为独立新 Skill。

### unknown（部分示例，共 37）
- agent-central-mapping：无 source 字段、无作者/上游引用。
- ai-coding-homework-writeup：无 source 字段、无作者/上游引用。
- ai-coding-homework-writeup (2)：『(2)』重复目录（2026-09-22 15:12 重扫发现，外部工具产生）：SKILL.md 内容与『ai-coding-homework-writeup』原目录逐字节一致（sha1 前12位同值），来源继承原目录判定；目录名含空格括号，目录扫描型加载器会将其识别为独立新 Skill。
- ai-quota-recovery-board：无 source 字段、无作者/上游引用。
- batch-organize：无 source 字段、无作者/上游引用。
- batch-organize (2)：『(2)』重复目录（2026-09-22 15:12 重扫发现，外部工具产生）：SKILL.md 内容与『batch-organize』原目录逐字节一致（sha1 前12位同值），来源继承原目录判定；目录名含空格括号，目录扫描型加载器会将其识别为独立新 Skill。
...
- coding-workspace-organizer (2)：『(2)』重复目录（2026-09-22 15:12 重扫发现，外部工具产生）：SKILL.md 内容与『coding-workspace-organizer』原目录逐字节一致（sha1 前12位同值），来源继承原目录判定；目录名含空格括号，目录扫描型加载器会将其识别为独立新 Skill。
- course-md-organize：无 source 字段、无作者/上游引用。（NOTE: central 与 60 Skill 仓库副本内容不同，见 CONTENT_VARIANTS。）
- course-md-organize (2)：『(2)』重复目录（2026-09-22 15:12 重扫发现，外部工具产生）：SKILL.md 内容与『course-md-organize』原目录逐字节一致（sha1 前12位同值），来源继承原目录判定；目录名含空格括号，目录扫描型加载器会将其识别为独立新 Skill。
- …（其余 25 个同口径：无作者来源证据）
...
- Obsidian: obsidian
- Orca: computer-use-2, orca-codex-commands
...
- 本地 Skill Manager: skill-manager-governance, skill-manager-governance (2)

## 总览统计

- 唯一 Skill：49
- 物理副本：65（冗余 16）+ 悬空链接 6 条
- 多平台/多目录副本（本轮实测）：browseros-neo×3（claude/opencode/agents 同内容实目录）；dida-task-manager 中央+workbuddy 平行实目录；13 个 skill 在 60 Skill 仓库有平行副本
- 『(2)』重复目录：7 个（2026-09-22 约14:23 外部工具在中央仓库产生；SKILL.md 内容与原目录逐字节一致；位于加载路径，按名称识别的加载器会视为新 Skill；本轮未动）
- 项目级平行副本：13 个 skill 在 60 Skill 仓库有副本（其中 1 个存在 SKILL.md 差异：实质内容变体 0 + 仅换行符差异 1；仓库副本不参与 agent 加载）
- 悬空符号链接：6（computer-use, orca-cli, orchestration）→ INSTALLATION broken_link；根因=外部工具移除中央 orca-cli/orchestration/computer-use 主副本，链接目标消失（内容档案保留于 SKILL_SOURCE_MAP.json）
- 内容变体（CONTENT_VARIANTS）：1（course-md-organize；其中仅换行符差异 1 个——规范化后内容一致）
- 本地修改（LOCAL_MODIFIED，可证明基于上游的修改）：0（判定需证明『本地内容是在官方基线上人工修改』；版本旧/与当前上游不一致不触发——见 VERSION_STATUS）
- 安装冲突（INSTALLATION_CONFLICT）：0（各 agent 加载路径上的副本内容经 SHA256 实测一致；内容变体≠安装冲突）
- 已确认来源：12/49；来源未知：37/49
...
- workbuddy:computer-use -> /Users/zzymima0000/.skills-manager/skills/computer-use
- claude:orca-cli -> /Users/zzymima0000/.skills-manager/skills/orca-cli
- claude:orchestration -> /Users/zzymima0000/.skills-manager/skills/orchestration
- claude:computer-use -> /Users/zzymima0000/.skills-manager/skills/computer-use
- opencode:orca-cli -> /Users/zzymima0000/.skills-manager/skills/orca-cli
- opencode:computer-use -> /Users/zzymima0000/.skills-manager/skills/computer-use
...
- course-md-organize：仅换行符差异（CRLF vs LF，规范化后内容一致，非实质变体）；repo 副本非加载路径 → 不构成安装冲突；可统一换行符后消除。

### ✏️ LOCAL_MODIFIED（可证明基于上游/原版本的本地修改）
- （无可证明项。精确口径：对能找到对应版本基线的 Skill 做内容比对——Orca 系与官方历史 commit 逐字节一致（historical_official_version）；hv-analysis/khazix-writer 与 khazix-skills 逐字节一致（upstream_in_sync）；browseros-neo 与所查 3 个官方 commit 均非逐字节一致（官方持续演进，outdated_version）；aihot 本地 1.2.0 落后官方 1.7.1（outdated_version）。**版本旧/与当前上游不一致 ≠ 本地修改**；『两份本地副本内容不同』只记 CONTENT_VARIANTS；无一能证明基于官方基线人工改动 → LOCAL_MODIFIED 为空。）
...
- identified（来源已确认）：12 个 — official 5（anthropics/skills×3 版权行；stablyai/orca×1=computer-use-2 与官方 commit 逐字节一致；browseros-ai/BrowserOS×1 官方管理清单；另 orca-cli/orchestration/computer-use 3 项已移出安装集）、vendor 1（aihot，文件内署名 Virxact）、community 2（hv-analysis/khazix-writer，具名作者 + khazix-skills 逐字节一致）、local 4（manage-skills/skill-manager-governance 及其『(2)』副本/place-journal-qa，本机项目自建）。
- unknown（未确认）：37 个。其中 orca-codex-commands 已排除官方发布；leader/neat-freak 与 khazix-skills 逐字节一致，待用户确认后升级（pending_upgrade）。
- UPSTREAM_ACTIVITY（活跃度，2026-09-22 联网核验）：active 8 ｜ stale 0 ｜ deprecated 0 ｜ not_applicable 4（local，无外部上游） ｜ unknown 37（来源未知项不做活跃推断）。active 判定依据均为实测 recent push/release，非『URL 能打开』。
- VERSION_STATUS（版本关系，独立于 LOCAL_MODIFIED）：historical_official_version 1（computer-use-2）｜ upstream_in_sync 4（hv-analysis、khazix-writer、leader、neat-freak）｜ outdated_version 2（browseros-neo 官方演进中、aihot 1.2.0→官方 1.7.1）｜ local_modified 0｜ unresolved_variant 0｜ not_compared 42（无上游内容基线，多为来源 unknown 项）。来源确认不要求与当前上游逐字节一致。
...
### ai-coding-homework-writeup (2)
- 用途："本 skill 适用于「把 AI 辅助开发的编程作业写成一份能交的提交材料」这类任务。当用户拿出作业要求（例如『用 Expo/React Native 开发一个 App，提交一句话介绍、运行截图、页面结构、核心功能、数据库说明』），要求你根据项目实际情况代写、帮交作业、填提交材料时触发；用户抱怨材料『写太长了』『像 GitHub 说明』『复制会乱码』『图片不要放中间』时同样触发。它提供事实核实流程、长度红线、富文本（非 Markdown）输出规范，以及两个跨平台脚本：扫描项目事实、把内容清单渲染成富文本 HTML。"
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a
  - 证据：无 source 字段、无作者/上游引用。（『(2)』重复目录：内容与原目录逐字节一致，来源继承）
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：skills-manager
- 多副本：否（1 份；内容一致）
- 能力标签：Documentation, Education, Other
...
- 健康与状态：
  ...
  - UPSTREAM_ACTIVITY: ⚪ unknown（未核验/无上游可查）
  - VERSION_STATUS: ➖ 无上游内容基线可比对
- 规范路径：/Users/zzymima0000/.skills-manager/skills/ai-coding-homework-writeup (2)
...
- 健康与状态：
  ...
  - VERSION_STATUS: 🟠 落后于当前上游（版本旧 ≠ 本地修改）
...
- 健康与状态：
  ...
  - UPSTREAM_ACTIVITY: ⚪ unknown（未核验/无上游可查）
  - VERSION_STATUS: ➖ 无上游内容基线可比对
- 规范路径：/Users/zzymima0000/.skills-manager/skills/batch-organize (2)
...
- 健康与状态：
  ...
  - VERSION_STATUS: 📜 与官方历史版本逐字节一致（官方已有更新）
- 与其他 Skill：
...
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：claude, opencode, skills-manager, workbuddy
- 多副本：是（2 份；内容存在差异；仅换行符差异（CRLF vs LF，规范化后内容一致，非实质变体））
...
### course-md-organize (2)
- 用途："把已提取的课程 Markdown 整理成「得到·刘澜式」舒服可读的文稿：顶部本课总结、错别字/OCR 校对、三级子标题逻辑分段、段落自然流动。用于 OCR/语音转写稿存在文字墙、错字、无结构的一批课程 .md，做批量、一致的重排版与精校。脚本阶段用 Python 标准库（无需依赖）；语义阶段由 LLM 执行。"
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a
  - 证据：无 source 字段、无作者/上游引用。（NOTE: central 与 60 Skill 仓库副本内容不同，见 CONTENT_VARIANTS。）（『(2)』重复目录：内容与原目录逐字节一致，来源继承）
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：skills-manager
- 多副本：否（1 份；内容一致）
- 能力标签：Documentation, Python, Other
...
- 健康与状态：
  ...
  - UPSTREAM_ACTIVITY: ⚪ unknown（未核验/无上游可查）
  - VERSION_STATUS: ➖ 无上游内容基线可比对
- 规范路径：/Users/zzymima0000/.skills-manager/skills/course-md-organize (2)
...
- 风险面引用：pip ×21, token×11, subprocess×9, .env×8, shutil×5, git ×1
- 本地修改（可证明）：未检出（无上游基线可比对）
- 健康与状态：
  - CONTENT_HEALTH: 🟡 需审阅 — 引用 secret/token（经 .env/MCP 注入，未见硬编码，需审阅）
  - INSTALLATION_HEALTH: ✅ 正常
...
### local-service-change-acceptance (2)
- 用途：改完本地长驻服务（单文件 Python HTTP 服务、本地控制台后端、桌面应用的内嵌服务）后，如何在真机上做「可信验收」——把用户真实数据撇开、用隔离目录跑端到端、以及识破「乐观状态位」「去重计数翻倍」「新字段没清零」这三类假通过/假失败。当用户说「改完了帮我验一下」「重启服务」「为什么状态显示不对」「验收不通过但看不出原因」时使用。
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a
  - 证据：无 source 字段、无作者/上游引用。（『(2)』重复目录：内容与原目录逐字节一致，来源继承）
- 安装位置：用户级（skills-manager 中央仓库）
...
- 健康与状态：
  ...
  - UPSTREAM_ACTIVITY: ⚪ unknown（未核验/无上游可查）
  - VERSION_STATUS: ➖ 无上游内容基线可比对
- 规范路径：/Users/zzymima0000/.skills-manager/skills/local-service-change-acceptance (2)
...
### skill-manager-governance (2)
- 用途：审查、修正、跨平台打包并正式导入本地 Skill Manager 技能库。用户要求把中转站 skill 纳入中央库、解释为什么软件看不到、或迁移到 Windows、macOS 或其他智能体时使用。
- 来源：local ｜ 上游：本地 Skill Manager / 60 Skill 仓库 ｜ confidence=high ｜ 目标平台：本地 Skill Manager
  - 证据：SKILL.md 自述『审查、修正、跨平台打包并正式导入本地 Skill Manager 技能库』——本机 Skill Manager 体系配套 skill。（『(2)』重复目录：内容与原目录逐字节一致，来源继承）
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：skills-manager
- 多副本：否（1 份；内容一致）
- 能力标签：Skill Development, Other
...
- 健康与状态：
  ...
  - UPSTREAM_ACTIVITY: ➖ not_applicable（本地自建，无外部上游） — 核验日 ；本地自建，无外部上游
  - VERSION_STATUS: ➖ 无上游内容基线可比对
- 规范路径：/Users/zzymima0000/.skills-manager/skills/skill-manager-governance (2)
...
- 健康与状态：
  ...
  - UPSTREAM_ACTIVITY: ⚪ unknown（未核验/无上游可查）
  - VERSION_STATUS: ➖ 无上游内容基线可比对
- 规范路径：/Users/zzymima0000/.skills-manager/skills/win-reveal-and-gui-selftest (2)
