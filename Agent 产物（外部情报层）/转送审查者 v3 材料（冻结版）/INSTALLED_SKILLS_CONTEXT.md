...
- 唯一 Skill 数：**49**（含 7 个『(2)』重复目录——文件系统真实存在、会被目录扫描型加载器识别为独立 Skill，性质见下）
- 实体物理副本：65（冗余 16）+ 悬空符号链接 6 条
- 用户级（skills-manager 中央仓库）：46 个含 SKILL.md 的目录
- 项目级（60 Skill 仓库平行开发副本）：13 个（其中 1 个存在 SKILL.md 差异——0 实质内容变体 + 1 仅换行符差异；均不参与 agent 加载）
- **已确认来源（作者归属有证据）：12**（official 5 ｜ vendor 1 ｜ community 2 ｜ local 4 ｜ fork 0 ｜ modified 0）
- **来源未知：37**（只能确认操作对象、无法证明作者归属；orca-codex-commands 已经官方全量 tree 检索排除官方发布）

### 重扫发现的安装层变化（13:55 → 15:12，外部工具造成；本轮只记录、未做任何修复）
- **被移除（3）**：orca-cli、orchestration、computer-use 主副本从中央仓库消失 → 6 条符号链接悬空（claude×3、opencode×2、workbuddy×1），这些 agent 路径已加载不到该 3 个 Skill；computer-use-2 未受影响、仍正常安装。
- **新增（7）**：『XXX (2)』重复目录 ×7（ai-coding-homework-writeup / batch-organize / coding-workspace-organizer / course-md-organize / local-service-change-acceptance / skill-manager-governance / win-reveal-and-gui-selftest），全部位于中央加载路径 ~/.skills-manager/skills/，SKILL.md 内容与原目录逐字节一致（SHA 实测）；目录名含空格括号，按名称识别的加载器会将其视为 7 个新 Skill。
- 处置建议（未执行）：确认后合并/清理 (2) 目录；决定是否恢复 3 个 Orca Skill 或清理 6 条悬空链接。


### 健康与状态
- CONTENT_HEALTH（内容）：healthy 19 ｜ warning 30 ｜ broken 0
- INSTALLATION_HEALTH（安装/加载路径）：healthy 49 ｜ broken_link 6（指向已被移除的 orca-cli/orchestration/computer-use；安装冲突单列 INSTALLATION_CONFLICT）
- UPSTREAM_STATUS（来源确认态）：identified 12 ｜ unknown 37（identified≠活跃，活跃度见 UPSTREAM_ACTIVITY）
- UPSTREAM_ACTIVITY（联网核验活跃度）：active 8 ｜ stale 0 ｜ deprecated 0 ｜ not_applicable 4（本地自建） ｜ unknown 37（未核验/无上游可查）｜ 核验日 2026-09-22
...
- workbuddy:computer-use -> /Users/zzymima0000/.skills-manager/skills/computer-use（内容本身正常，仅该 agent 目录加载不到；修复=更新符号链接目标）
- claude:orca-cli -> /Users/zzymima0000/.skills-manager/skills/orca-cli（内容本身正常，仅该 agent 目录加载不到；修复=更新符号链接目标）
- claude:orchestration -> /Users/zzymima0000/.skills-manager/skills/orchestration（内容本身正常，仅该 agent 目录加载不到；修复=更新符号链接目标）
- claude:computer-use -> /Users/zzymima0000/.skills-manager/skills/computer-use（内容本身正常，仅该 agent 目录加载不到；修复=更新符号链接目标）
- opencode:orca-cli -> /Users/zzymima0000/.skills-manager/skills/orca-cli（内容本身正常，仅该 agent 目录加载不到；修复=更新符号链接目标）
- opencode:computer-use -> /Users/zzymima0000/.skills-manager/skills/computer-use（内容本身正常，仅该 agent 目录加载不到；修复=更新符号链接目标）
...
- course-md-organize：仅换行符差异（CRLF vs LF，规范化后内容一致，非实质变体）（repo 副本非加载路径 → 非安装冲突；建议统一换行符）

### ✏️ 本地修改（LOCAL_MODIFIED，可证明基于上游/原版本的本地修改）
- （无可证明项。精确口径：对能找到对应版本基线的 Skill 做内容比对——Orca 系与官方历史 commit 逐字节一致（historical_official_version）；hv-analysis/khazix-writer 与 khazix-skills 逐字节一致（upstream_in_sync）；browseros-neo 与所查官方 commit 均非逐字节一致（官方持续演进，outdated_version）；aihot 本地 1.2.0 落后官方 1.7.1（outdated_version）。**版本旧/与当前上游不一致 ≠ 本地修改**；无一能证明基于官方基线人工改动 → LOCAL_MODIFIED 为空）
...
- identified（来源已确认）：12（official 5 / vendor 1 / community 2 / local 4；orca-cli/orchestration/computer-use 已被移出安装集，official 相应减少）
- unknown（未确认）：37（不代表失效；orca-codex-commands 已排除官方发布，leader/neat-freak 待用户确认后可升级）
- UPSTREAM_ACTIVITY：active 8（均经 2026-09-22 联网核验：anthropics/skills push 2026-09-10、stablyai/orca 与 browseros-ai/BrowserOS push 2026-09-22、KKKKhazix/khazix-skills push 2026-09-16、aihot 官方渠道 v1.7.1 持续更新）｜ not_applicable 4（local 自建，无外部上游）｜ unknown 37（来源未知项不做活跃推断）
- VERSION_STATUS（版本关系，不触发 LOCAL_MODIFIED）：historical_official_version 1（computer-use-2）｜ upstream_in_sync 4（hv-analysis、khazix-writer、leader、neat-freak）｜ outdated_version 2（browseros-neo、aihot）｜ local_modified 0｜ not_compared 42
...
CONTENT_VARIANTS:   # 同一 Skill 多位置内容版本不同（动态实测 SHA256）
  - course-md-organize   # 仅换行符差异（CRLF vs LF，规范化后内容一致，非实质变体）；repo 副本非加载路径 → 非安装冲突
...
INSTALLATION_HEALTH:   # 仅看加载路径；内容变体单列 CONTENT_VARIANTS，不在此计冲突
  healthy: 49
  broken_link: 6   # claude×3 / opencode×2 / workbuddy×1，目标为被移除的 Orca 主副本
  path_error: 0
  conflict: 0
...
UPSTREAM_ACTIVITY:   # active 仅在联网核验后填写；not_applicable=本地自建无上游；unknown=来源未知项不做推断
  ...
  active_items: aihot, browseros-neo, computer-use-2, frontend-design, hv-analysis, khazix-writer, skill-creator, theme-factory

VERSION_STATUS:   # 与上游版本关系；版本旧/与当前上游不一致 ≠ LOCAL_MODIFIED（需证明基于官方基线人工修改才记 LOCAL_MODIFIED）
  historical_official_version: 1   # 与官方历史 commit 逐字节一致，官方已有更新
  upstream_in_sync: 4   # 与上游当前内容逐字节一致
  outdated_version: 2   # 落后于当前上游（browseros-neo 官方演进；aihot 1.2.0 vs 1.7.1）
  local_modified: 0
  unresolved_variant: 0
  not_compared: 42   # 无上游内容基线可比对（多为来源 unknown 项）

NEEDS_ATTENTION:
  - 悬空符号链接(INSTALLATION broken_link): computer-use, orca-cli, orchestration   # 根因=外部工具移除中央 3 个 Orca 主副本
  - 『(2)』重复目录(DUP): ai-coding-homework-writeup (2), batch-organize (2), coding-workspace-organizer (2), course-md-organize (2), local-service-change-acceptance (2), skill-manager-governance (2), win-reveal-and-gui-selftest (2)   # 内容与原目录逐字节一致，位于中央加载路径，按名称识别的加载器会当作新 Skill；建议确认后清理（本轮未动）
  - orca-cli/orchestration/computer-use 被外部工具移除: 能力 orca_integration 的载体缺失，等待用户决定恢复或删除能力项
  - 内容差异待对齐(CONTENT_VARIANTS): course-md-organize   # 仅换行符差异（CRLF vs LF，规范化后内容一致，非实质变体）
  - 来源未知 37 个(UPSTREAM_STATUS unknown)：建议补登 source 字段（勿按名称猜）
...
| ai-coding-homework-writeup (2) | "本 skill 适用于「把 AI 辅助开发的编程作业写成一份能交的提交材料」这类任务。当用户拿出作业要求（例如『用 Expo/React Native 开发一个 App，提交一句… | 用户级 | unknown | Documentation,Education,Other | ✅ | ✅ | ⚪ | ⚪ |
...
| batch-organize (2) | "本 skill 适用于「一批杂乱原始素材 → 统一规范成品」且需可重跑、可中断、可多智能体协作的批量整理任务（课程/播客/视频转写稿清洗、PDF 抽取稿整理、文档/知识库迁移、素… | 用户级 | unknown | Agent/Workflow,Documentation,Other | ✅ | ✅ | ⚪ | ⚪ |
...
| coding-workspace-organizer (2) | 整理编码工作区：拍平嵌套归档目录，按 `NNN-状态-项目名` 规则重命名项目，并生成按序号查找的索引表。用户要求整理工作区、给项目文件夹编号、按序号排序、拍平暂停或归档目录，或在… | 用户级 | unknown | macOS,Windows,Automation,Other | ✅ | ✅ | ⚪ | ⚪ |
| computer-use-2 | Use Orca's computer-use CLI for OS/window-level inspection and input in visible local app … | 用户级 | official | Agent/Workflow,macOS,Windows,Browser Automation | ✅ | ✅ | 🔵 | 🟢 |
...
| course-md-organize (2) | "把已提取的课程 Markdown 整理成「得到·刘澜式」舒服可读的文稿：顶部本课总结、错别字/OCR 校对、三级子标题逻辑分段、段落自然流动。用于 OCR/语音转写稿存在文字墙、… | 用户级 | unknown | Documentation,Python,Other | 🟡 | ✅ | ⚪ | ⚪ |
...
| local-service-change-acceptance (2) | 改完本地长驻服务（单文件 Python HTTP 服务、本地控制台后端、桌面应用的内嵌服务）后，如何在真机上做「可信验收」——把用户真实数据撇开、用隔离目录跑端到端、以及识破「乐观… | 用户级 | unknown | QA/Testing,Other | ✅ | ✅ | ⚪ | ⚪ |
...
| neat-freak | Knowledge and governance closeout: reconcile project docs, rule files (CLAUDE.md/AGENTS.md… | 用户级 | unknown | Documentation,Agent/Workflow,Other | 🟡 | ✅ | ⚪ | ⚪ |
...
| orca-codex-commands | Use when Stage Manager needs to call Codex via Orca terminal. Provides the exact commands … | 用户级 | unknown | Agent/Workflow,Git/GitHub,Other | ✅ | ✅ | ⚪ | ⚪ |
...
| skill-manager-governance (2) | 审查、修正、跨平台打包并正式导入本地 Skill Manager 技能库。用户要求把中转站 skill 纳入中央库、解释为什么软件看不到、或迁移到 Windows、macOS 或其… | 用户级 | local | Skill Development,Other | 🟡 | ✅ | 🔵 | ➖ |
...
| win-reveal-and-gui-selftest (2) | 在 Windows 上实现「打开文件所在位置／在资源管理器中定位文件」，以及用窗口标题差分自测任何 GUI 动作是否真的发生（含沙箱里跑 Bash/PowerShell 的进程与窗… | 用户级 | unknown | Windows,QA/Testing,Other | 🟡 | ✅ | ⚪ | ⚪ |
