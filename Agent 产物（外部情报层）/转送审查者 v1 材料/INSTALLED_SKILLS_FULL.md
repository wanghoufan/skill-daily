...
方法（第二轮修订）：只认**作者来源证据**——official=官方仓库/官方 LICENSE 版权/官方发布记录；vendor=明确证据表明 Skill 由该厂商/团队提供；community=可确认第三方作者/社区来源；local=明确属于本机/用户项目自研；unknown=只能确认『操作某产品』但无法确认作者。
...
第一版修订记录：orca-cli / computer-use / computer-use-2 / orchestration / orca-codex-commands / browseros-neo 曾因『面向 Orca/BrowserOS』判 vendor、aihot 曾因『调用其 API』判 community，均证据不足，已降回 unknown。

- 已确认来源（作者归属有证据）：**8**（official 3 ｜ vendor 0 ｜ community 2 ｜ local 3 ｜ fork 0 ｜ modified 0）
- 来源未知：**37**（含上述 7 个降级项）
...
### community（2）
- **hv-analysis** ← 数字生命卡兹克 (Khazix) ｜ confidence=medium ｜ 证据：SKILL.md 明确标注方法由第三方作者『数字生命卡兹克』提出——内容来源可确认；skill 文件本身未直接署名，故 confidence=medium。
- **khazix-writer** ← 数字生命卡兹克 (Khazix) ｜ confidence=high ｜ 证据：SKILL.md 自述：『数字生命卡兹克（Khazix）的个人写作风格skill』——直接署名。
...
- aihot：SKILL.md 仅证明必须调用 aihot.virxact.com 的匿名只读 API（服务依赖≠作者归属）；无法证明由该服务方编写 → 第一版 community 判定撤销。
...
- browseros-neo：SKILL.md 仅证明面向 BrowserOS 浏览器产品（'the user's dedicated browser for agents'）；不能证明由 BrowserOS 团队发布 → 第一版 vendor 判定撤销。
- coding-workspace-organizer：无 source 字段、无作者/上游引用。
- computer-use：SKILL.md 仅证明『通过 Orca computer-use CLI 操作桌面窗口』（面向产品）；不能证明由 Orca 厂商发布 → 第一版 vendor 判定撤销。
- computer-use-2：SKILL.md 仅证明面向 Orca computer-use CLI 的细化版（面向产品）；无作者归属证据 → 第一版 vendor 判定撤销。
...
- 本地修改（LOCAL_MODIFIED，可证明基于上游的修改）：0（判定需上游基线，本轮未联网取上游版本，无可证明项）
- 安装冲突（INSTALLATION_CONFLICT）：0（各 agent 加载路径上的副本内容经 SHA256 实测一致；内容变体≠安装冲突）
- 已确认来源：8/45；来源未知：37/45
...
- （无可证明项：判定需先确认上游基线并比对，本轮未联网取上游版本；『两份本地副本内容不同』只记 CONTENT_VARIANTS，不自动记 LOCAL_MODIFIED。）
...
### ⚪ UPSTREAM_STATUS（来源确认态）
- identified（来源已确认）：8 个 — official 3（frontend-design/theme-factory/skill-creator，LICENSE 版权为证）、community 2（hv-analysis/khazix-writer，具名作者）、local 3（manage-skills/skill-manager-governance/place-journal-qa，本机项目自建）。vendor 0：无一能证明『由厂商发布』。
- unknown（未确认）：37 个。**本轮无任何 active 判定**：identified 只表示作者归属确认，活跃度（active/stale/deprecated）需联网核验后另设 UPSTREAM_ACTIVITY 字段。
...
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a ｜ 目标平台：AI HOT (aihot.virxact.com)
  - 证据：SKILL.md 仅证明必须调用 aihot.virxact.com 的匿名只读 API（服务依赖≠作者归属）；无法证明由该服务方编写 → 第一版 community 判定撤销。
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：skills-manager
- 多副本：否（1 份；内容一致）
- 能力标签：Research, Other
...
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a ｜ 目标平台：BrowserOS
  - 证据：SKILL.md 仅证明面向 BrowserOS 浏览器产品（'the user's dedicated browser for agents'）；不能证明由 BrowserOS 团队发布 → 第一版 vendor 判定撤销。
- 安装位置：用户级（agent 目录）
- 可用 Agent：claude
- 多副本：是（3 份；内容一致）
...
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a ｜ 目标平台：Orca
  - 证据：SKILL.md 仅证明『通过 Orca computer-use CLI 操作桌面窗口』（面向产品）；不能证明由 Orca 厂商发布 → 第一版 vendor 判定撤销。
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：claude, opencode, skills-manager, workbuddy
- 多副本：否（1 份；内容一致）
...
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a ｜ 目标平台：Orca
  - 证据：SKILL.md 仅证明面向 Orca computer-use CLI 的细化版（面向产品）；无作者归属证据 → 第一版 vendor 判定撤销。
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：skills-manager
- 多副本：否（1 份；内容一致）
- 能力标签：Agent/Workflow, macOS, Windows, Browser Automation
...
- 健康与状态：
  - CONTENT_HEALTH: 🟡 需审阅 — 引用 secret/token（经 .env/MCP 注入，未见硬编码，需审阅）
  - INSTALLATION_HEALTH: ✅ 正常
  - UPSTREAM_STATUS: 🔵 已识别 — 来源已确认：official / github.com/anthropics/skills（identified 仅指作者归属确认，未做在线活跃度核验）
- 与其他 Skill：
...
- 来源：community ｜ 上游：数字生命卡兹克 (Khazix) ｜ confidence=medium
  - 证据：SKILL.md 明确标注方法由第三方作者『数字生命卡兹克』提出——内容来源可确认；skill 文件本身未直接署名，故 confidence=medium。
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：skills-manager, workbuddy
- 多副本：否（1 份；内容一致）
...
- 健康与状态：
  - CONTENT_HEALTH: 🟡 需审阅 — 依赖网络/外部 CLI，离线环境可能受限
  - INSTALLATION_HEALTH: ✅ 正常
  - UPSTREAM_STATUS: 🔵 已识别 — 来源已确认：community / 数字生命卡兹克 (Khazix)（identified 仅指作者归属确认，未做在线活跃度核验）
- 与其他 Skill：
  - 与 khazix-writer 互补
...
- 来源：community ｜ 上游：数字生命卡兹克 (Khazix) ｜ confidence=high
  - 证据：SKILL.md 自述：『数字生命卡兹克（Khazix）的个人写作风格skill』——直接署名。
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：skills-manager
- 多副本：否（1 份；内容一致）
...
- 健康与状态：
  - CONTENT_HEALTH: 🟡 需审阅 — 依赖网络/外部 CLI，离线环境可能受限
  - INSTALLATION_HEALTH: ✅ 正常
  - UPSTREAM_STATUS: 🔵 已识别 — 来源已确认：local / 本地 skills-manager CLI（identified 仅指作者归属确认，未做在线活跃度核验）
- 与其他 Skill：
...
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a
  - 证据：无 source 字段、无作者/上游引用（v3.0.0，仅泛引 Agent Skills 标准）。
- 安装位置：用户级（skills-manager 中央仓库）
...
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a ｜ 目标平台：Orca
  - 证据：SKILL.md 仅证明『操作 Orca CLI / Orca-managed worktrees』（面向产品）；无任何作者/发布方证据 → 第一版 vendor 判定撤销。
- 安装位置：用户级（skills-manager 中央仓库）
- 可用 Agent：claude, opencode, skills-manager
- 多副本：否（1 份；内容一致）
...
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a ｜ 目标平台：Orca
  - 证据：SKILL.md 仅证明面向 Orca 终端调用 Codex 的命令集（面向产品）；无作者归属证据 → 第一版 vendor 判定撤销。
- 安装位置：用户级（agent 目录）
- 可用 Agent：opencode
- 多副本：否（1 份；内容一致）
...
- 来源：unknown ｜ 上游：unknown ｜ confidence=n/a ｜ 目标平台：Orca
  - 证据：SKILL.md 仅证明『使用 Orca orchestration 做多 agent 协调』（面向产品）；无作者归属证据 → 第一版 vendor 判定撤销。
- 安装位置：用户级（skills-manager 中央仓库）
...
- 健康与状态：
  - CONTENT_HEALTH: ✅ 正常
  - INSTALLATION_HEALTH: ✅ 正常
  - UPSTREAM_STATUS: 🔵 已识别 — 来源已确认：local / place-journal 项目（identified 仅指作者归属确认，未做在线活跃度核验）
...
- 健康与状态：
  - CONTENT_HEALTH: 🟡 需审阅 — 引用 secret/token（经 .env/MCP 注入，未见硬编码，需审阅）
  - INSTALLATION_HEALTH: ✅ 正常
  - UPSTREAM_STATUS: 🔵 已识别 — 来源已确认：local / 本地 Skill Manager / 60 Skill 仓库（identified 仅指作者归属确认，未做在线活跃度核验）
- 与其他 Skill：
