...
- **来源未知：37**（只能确认操作对象、无法证明作者归属；含 7 个由『面向产品』误判降回的 skill）
...
- UPSTREAM_STATUS（来源确认态）：identified 8 ｜ unknown 37（**无 active**：本轮未联网核验活跃度，identified≠活跃）
...
- （无可证明项：判定需先有上游基线可比对，本轮未联网取上游版本；内容变体不自动等于本地修改）
...
### ⚪ 来源层（UPSTREAM_STATUS）
- identified（来源已确认）：8（official 3 / community 2 / local 3；vendor 0）
- unknown（未确认）：37（**不代表失效**；identified 也不代表活跃——活跃度需联网核验，本轮未做）
...
UPSTREAM_STATUS:   # identified=作者归属已确认（≠活跃）；活跃度核验属 UPSTREAM_ACTIVITY，本轮未做
  identified: 8
  unknown: 37
...
| Skill | 用途(简述) | 级别 | 来源 | 标签 | 内容 | 安装 | 来源态 |
|---|---|---|---|---|---|---|---|
...
| aihot | 查询 AI HOT 的中文 AI 资讯、精选、当前热点和日报。用户询问今天或最近的 AI 新闻、AI 圈动态、大模型或产品发布、OpenAI／Anthropic／Google 最新… | 用户级 | unknown | Research,Other | 🟡 | ✅ | ⚪ |
...
| browseros-neo | The user's dedicated browser for agents — a real browser signed into their accounts, with … | 用户级 | unknown | Browser Automation,MCP,Other | ✅ | ✅ | ⚪ |
...
| computer-use | - Use Orca's computer-use CLI to inspect and operate local desktop app windows through acc… | 用户级 | unknown | Agent/Workflow,macOS,Windows,Browser Automation | ✅ | ✅ | ⚪ |
| computer-use-2 | - Use Orca's computer-use CLI for OS/window-level inspection and input in visible local ap… | 用户级 | unknown | Agent/Workflow,macOS,Windows,Browser Automation | ✅ | ✅ | ⚪ |
...
| orca-cli | - Use the public `orca` CLI to operate Orca-managed worktrees, folder contexts, terminals,… | 用户级 | unknown | Agent/Workflow,macOS,Windows,Git/GitHub | ✅ | ✅ | ⚪ |
...
| orchestration | - Use Orca orchestration for structured multi-agent coordination: threaded messages, block… | 用户级 | unknown | Agent/Workflow,Other | ✅ | ✅ | ⚪ |
...
> 图例：内容=CONTENT_HEALTH(✅正常/🟡需审阅/🔴损坏)；安装=INSTALLATION_HEALTH(✅正常/🔴悬空链接)；来源态=UPSTREAM_STATUS(🔵来源已确认 identified/⚪未确认 unknown，均不代表在线活跃)。
